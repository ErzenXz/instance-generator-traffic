...
function showMap(data, start = "06:00", end = "17:45", mapStyle, selectedIntersection) {
   ...
   if (selectedIntersection) {
      map.setView([selectedIntersection.lat, selectedIntersection.lon], 40);
   } else {
   ...
   for (var i = 0; i < data.length; i++) {
      ...
      var popup = L.popup().setContent(
         `<h3>${name} <span style="color: ${c}">•</span></h3>` +
            `<br><strong>` +
            totalCars +
            "</strong> cars have passed here from <i>" +
            start +
            "</i> until <i>" +
            end +
            "</i> o'clock. <br> Location: " +
            `<a target="_blank" href="https://maps.google.com/?q=${location.lat},${location.lon}">${location.lat},${location.lon}</a>` +
            "."
      );
   ...
   document.getElementById(
      "totalCars"
   ).innerHTML = `<strong>${ttCars}</strong> cars have circulated in Prishtina from <i>${start}</i> until <i>${end}</i> o'clock.`;
